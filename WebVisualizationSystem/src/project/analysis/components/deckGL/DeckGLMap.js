...
class DeckGLMap extends Component {
  constructor(props) {
    ...
    this.iconLayerShow = false;//是否显示OD点的ICON图标
    this.state = {
  ...
  getTripsLayer = (selectData) => {
    this.setState({
      // 获取轨迹数，用于展示tripsLayers时选择渲染效果
      dataLength: this.props.userData.length
    },
      () => {
    ...
    this.setState(
      {
        iconLayer: new IconLayer({
          ...
          // }),
          sizeScale: 10,
          onClick: info => this.clickIcon(info),
          getPosition: d => d.COORDINATES,
          getSize: d => 2,
  ...
  changeTripsLayerShow = () => {//与开关联动，切换OD弧段图层的显示与否
    this.tripsLayerShow = !this.tripsLayerShow;
    this.getTripsLayer(this.props.userData);
  };
  changeIconLayerShow = () => {//与开关联动，切换OD弧段图层的显示与否
  ...
  getLayers = () => {//获取所有图层
    ...
    this.getAllOdNodes();//获取所有OD
    this.getTripsLayer(this.props.userData);//构建轨迹图层
    this.getIconLayer(this.OdNodes);//构建OD图层
  };
  // 根据日期选择轨迹，监听函数
    ...
    return (
      <div>
        <DeckGL
          ...
          this.state.arcLayerOne, this.state.tripsLayerOne]}>
        ...
        <div className={`moudle`}>
          ...
          IconLayer    <Switch onChange={this.changeIconLayerShow} /><br />
        </div><br />
